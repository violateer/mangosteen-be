...
echo 'docker run ...'
docker run -e RAILS_MASTER_KEY=$RAILS_MASTER_KEY -d -p 3000:3000 --network=network1 -e DB_PASSWORD=$DB_PASSWORD -e DB_HOST=$DB_HOST --name=$container_name mangosteen:$version
echo 'DONE!'